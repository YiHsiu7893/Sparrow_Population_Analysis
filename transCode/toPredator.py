...
def extractLoc(locality):
    #print(locality)
    if locality == 'unknown' or pd.isna(locality):#or locality == 'District,' or locality == 'Township,':
...
df = pd.read_csv('inputData/白尾八哥.csv', encoding='big5', encoding_errors='ignore')
...
# Save to .csv files.
record.to_csv('finalData/predator.csv', index=False)
#record.to_csv('finalData/predator.csv', mode='a', index=False, header=False)
species.to_csv('finalData/species.csv', mode='a', index=False, header=False)